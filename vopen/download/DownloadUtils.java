...
public class DownloadUtils {
	 ...
	 * @param pnum
	 */
	public static void renameCompletedFile(Context context, String plid, int pnum) {
		String oldName = FileUtils.getSavedDownloadVideoPath(context,plid, pnum, false);
	 ...
	 * @throws IOException
	 */
	public static File getSaveFile(Context context, String plid, int pnum, boolean hasDownloadThreadInfo) throws IOException {
		String ncFileStr = FileUtils.getSavedDownloadVideoPath(context,plid, pnum, false);
		if(!TextUtils.isEmpty(ncFileStr)){
			...
			if(hasDownloadThreadInfo){
				for(int i = 0; i < Constants.Download_Thread_Count; i++){
					DownloadPrefHelper.recordDownload(context, plid, pnum, i, 0);
				}
	 ...
	 * @param pnum
	 */
	public static void renameCompleteSubFile(Context context, String plid, int pnum, int subIndex, String url) {
		String oldName = FileUtils.getSavedDownloadSubPath(context,plid, pnum, subIndex, url, false, false);
		if(oldName != null && oldName.length() > 3){
			String newName = oldName.substring(0, oldName.length() - 3);
			FileUtils.renameFile(newName, new File(oldName));
		}
	 ...
	 * @throws IOException
	 */
	public static File getSubSaveFile(Context context, String plid, int pnum, int subIndex, String url) throws IOException {
		File fileSaveDir = new File(FileUtils.getSavingDownloadFileDir(context));
		if (!fileSaveDir.exists()) {
			fileSaveDir.mkdirs();
		}
		File saveFile = new File(fileSaveDir, plid + "_" + pnum + "_" + subIndex + "_" + url.hashCode()
	 ...
	 * 
	 * */
	public static HttpResponse httpDownload(HttpClient httpClient, String url, int startPos, int endPos, 
			...
			headers[5] = new BasicHeader("User-Agent",
     ...
     * 获取文件大小
     */
	public static int getHttpFileSize(Context context, String url, List<NameValuePair> params, String encoding) {
		    int length = 0;
			...
			try {
				...
				HttpResponse response = HttpUtils.doHttpExecute(httpClient, url, params, headers, HttpUtils.GET, encoding);
				if(response == null)length = -2;//网络错误
...
//					}
				...
				PalLog.i("DownloadUtils", "getHttpFileSize Exception " + e.toString());
				e.printStackTrace();
				length = -2;//网络错误
			} finally {
				if (null != httpClient) {
	 ...
	 * @param total
	 */
	public static void sendCommpleteBroadcast(Context context, int status, int id, int down, int total) {
	...
	public static HttpResponse httpDownload(HttpClient httpClient, String url,  
			...
			headers[2] = new BasicHeader("Referer", url);
			headers[3] = new BasicHeader("Charset", "UTF-8");
			headers[4] = new BasicHeader("User-Agent",
					"Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.2; Trident/4.0; .NET CLR 1.1.4322; "
							+ ".NET CLR 2.0.50727; .NET CLR 3.0.04506.30; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729)");
	 ...
	 * @param id
	 * */
	public static EDownloadStatus downloadSubFile(Context context,String url,String plid,int pnum,int subIndex){
		EDownloadStatus result = EDownloadStatus.DOWNLOAD_DONE;
		AndroidHttpClient httpClient = HttpUtils.getAndroidHttpClient(context);
		File saveFile = null;
		try {
			saveFile = DownloadUtils.getSubSaveFile(context, plid,pnum,subIndex,url);
			PalLog.v("DownloadUtils", "downloadSubFile url= " + url + " saveFile = " + saveFile);
...
//						Thread.sleep(10);
			...
			PalLog.v("DownloadUtils", e.toString());
			long sdSize = saveFile == null ? 0 : SystemUtils.readSDCardRemainSize(saveFile.getAbsolutePath());
		...
		if(result.value() == EDownloadStatus.DOWNLOAD_DONE.value()){
			DownloadUtils.renameCompleteSubFile(context,plid,pnum,subIndex, url);
		}
		return result;
	}
}
