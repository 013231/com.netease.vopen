...
public class LoginTransaction extends BaseTransaction
{
	...
	String mVerifier;
	private boolean mGetTokenCalled = false;
...
//		}
    ...
    public void onResponseSuccess(String response)
	{
		...
		case STAT_REQUEST_TOKEN:
			...
			try
			{
				...
				NeteaseService.getInstance().setOauthToken(oauth_token, oauth_token_secret);				
			} catch (Exception e)
			{
				e.printStackTrace();
				notifyError(ErrDescrip.ERR_PARSE, new ErrDescrip(WeiboAccountColumn.WB_TYPE_NETEASE, ErrDescrip.ERR_PARSE, null, null));
				mStat = STAT_END;
			}
			break;
		...
		case STAT_VERIFY:
			...
			if(result != null){
				...
				String token = NeteaseService.getInstance().getOauthClient().mTokenKey;
				String secret = NeteaseService.getInstance().getOauthClient().mTokenSecret;
				ManagerWeiboAccount.setWBAccountToken(BaseApplication.getAppInstance(), result.getName(), 
												WeiboAccountColumn.WB_TYPE_NETEASE, token, secret, 
    ...
    public void onTransact()
	{
		...
		case STAT_AUTHENTICAT:
			...
			break;
		case STAT_ACCESS_TOKEN:
//		    Log.e("LoninTransaction", "mVerifier is " + mVerifier);
	...
	private LoginResult parseJson(String json){
		LoginResult result = new LoginResult(WeiboAccountColumn.WB_TYPE_NETEASE);
		...
		try {
			...
			result.setDomain("http://t.163.com/" + result.getScreeName());
	...
	private HttpRequest createRequestToken(){
		...
		String host = NeteaseService.getInstance().getRequestUrl(HOST_REQUEST_TOKEN);
		return NeteaseService.getInstance().getOauthClient().request(THttpMethod.GET,host, null);
	}

	private HttpRequest createAuthenticate(){
		String host = NeteaseService.getInstance().getRequestUrl(HOST_AUTHENTICATE);
		...
		url.append("=");
		url.append(NeteaseService.getInstance().getOauthClient().mTokenKey);
		url.append("&");
		...
		Authenticate auth = result.new Authenticate(url.toString(), RedirectUrl.OAUTH_CALLBACK) {
			...
			public void onGetToken(String token, String verifier,String uid) {
			    Log.i("LoginTransaction", "onGetToken " + mStat);
                if (!mGetTokenCalled) {
                    mGetTokenCalled = true;
...
//			}
	...
	private HttpRequest createAccessToken(String verifier){
		...
		String host = NeteaseService.getInstance().getRequestUrl(HOST_ACCESS_TOKEN);
		return NeteaseService.getInstance().getOauthClient().request(THttpMethod.GET, host, parameter);
	}
	...
	public HttpRequest createVerify(){
		...
		String url = NeteaseService.getInstance().getRequestUrl(URL_VERIFY);
		return NeteaseService.getInstance().getOauthClient().request(THttpMethod.GET,url, null);
	}
