...
public class BlogTransaction extends BaseTransaction {
	...
	private boolean isLogin(){
		String[] tokens = ManagerWeiboAccount.getWBAccountToken(BaseApplication.getAppInstance(),mName, WeiboAccountColumn.WB_TYPE_TENCENT);
	...
	public void onTransact() {
		...
		case TRANSACTION_TYPE_BLOGSEND:
				mHttpRequest = createBlogSend(mContent, mLatitude, mLongitude, mImagePath);
			break;
...
//		}
	...
	private Object parseJson(String json){
		SendBlogResult result = new SendBlogResult(WeiboAccountColumn.WB_TYPE_TENCENT);
...
//		else{
			...
			if (!BaseUtil.isStringEmpty(latitude)) {
				parameter.put("wei", latitude);
			}
			if (!BaseUtil.isStringEmpty(longitude)) {
				parameter.put("jing", longitude);
			}
...
//				parameter.put("pic", mImagePath);
				AddPicHttpRequest request = new AddPicHttpRequest(url, 
						THttpMethod.POST, 
						QqService.getInstance().getOauthClient());
				if (parameter != null) {
				...
				OAuthHttpRequest request = QqService.getInstance().getOauthClient().request(THttpMethod.POST, url, parameter);
...
//			parts.add(new StringPart("content", mContent, "UTF-8"));
			try {
				parts.add(new FilePart("pic", mImagePath,new File(mImagePath),null,null));//注：腾讯微博上传图片一定要有文件名，否则会返回size error
			} catch (FileNotFoundException e) {
				e.printStackTrace();
			}
